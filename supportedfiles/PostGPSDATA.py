...
def main():
    ...
    for i in range(0,100):
        ...
        tlog, tlat = GetRandomPath(-79.8, 36.8, 0.1)
        gpsdata = '{"gps":[{"stamp":' + str(i) + ',"timestamp":' + str(int(time.time())) + ',"lat":' + str(
            tlat) + ',"long":' + str(tlog) + '}]}'
        r = requests.patch('http://127.0.0.1:8000/gpsdatas/drone_test1/', auth=(username, password),
                           data={'deviceid': 'drone_test1', 'taskid': 'sar_put', 'gpsdata': gpsdata})

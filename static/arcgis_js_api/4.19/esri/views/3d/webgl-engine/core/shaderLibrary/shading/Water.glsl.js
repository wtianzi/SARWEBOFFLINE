...
//>>built
define("exports ../../shaderModules/interfaces ./ScreenSpaceReflections.glsl ./FoamRendering.glsl ./Gamma.glsl ./PhysicallyBasedRendering.glsl".split(" "),function(d,c,e,f,g,h){d.Water=function(a,b){a.include(h.PhysicallyBasedRendering,b);a.include(g.Gamma);a.include(f.FoamColor);b.ssrEnabled&&a.include(e.ScreenSpaceReflections,b);a.fragment.constants.add("fresnelSky","vec3",[.02,1,15]).add("fresnelMaterial","vec2",[.02,.1]).add("roughness","float",.015).add("foamIntensityExternal","float",1.7).add("ssrIntensity",
"float",.65).add("ssrHeightFadeStart","float",3E5).add("ssrHeightFadeEnd","float",5E5).add("waterDiffusion","float",.775).add("waterSeeColorMod","float",.8).add("correctionViewingPowerFactor","float",.4).add("skyZenitColor","vec3",[.52,.68,.9]).add("skyColor","vec3",[.67,.79,.9]);a.fragment.code.add(c.glsl`

...
function(e,d,h,k,l,m,n,p,q,r,t,u,v,w){function f(b){const a=new h.ShaderBuilder,c=a.vertex,x=a.fragment;b.legacy&&c.uniforms.add("uModel","mat4");b.antialiasing&&(c.code.add(d.glsl`
      #define ANTIALIASING 1
    `),x.code.add(d.glsl`
      #define ANTIALIASING 1
    `));a.include(n.AdjustProjectedPosition,b);a.include(v.LineAmplitude,b);a.include(t.EdgeUtil,b);a.include(u.UnpackAttributes,b);a.include(w.LineOffset,b);a.include(k.Slice,b);a.include(q.DiscardNonSilhouetteEdges,b);a.include(p.DiscardByCoverage,b);a.include(r.DiscardShortEdges,b);a.varyings.add("vColor","vec4");a.varyings.add("vRadius","float");a.varyings.add("vPosition","vec3");a.varyings.add("vWorldPosition","vec3");a.varyings.add("vViewPos","vec3");a.varyings.add("vLineLengthPixels","float");
a.varyings.add("vSizeFalloffFactor","float");c.uniforms.add("uPixelToNDC","vec2");c.uniforms.add("uNDCToPixel","vec2");c.uniforms.add("uPixelRatio","float");a.attributes.add("position0","vec3");a.attributes.add("position1","vec3");a.attributes.add("variantOffset","float");a.attributes.add("variantStroke","float");a.attributes.add("variantExtension","float");c.code.add(d.glsl`
  ...
  `);b.multipassTerrainEnabled&&(a.fragment.include(l.ReadLinearDepth),a.include(m.multipassTerrainTest,b));a.fragment.code.add(d.glsl`
  ...
  `);return a}const g={position0:0,position1:1,componentIndex:2,packedAttributes:3,variantOffset:4,variantStroke:5,variantExtension:6,normal:7,normalA:7,normalB:8,sideness:9};var y=Object.freeze({__proto__:null,attributeLocations:g,build:f});e.EdgeShaderProgram=y;e.attributeLocations=g;e.build=f});
...
function(f,c,e,l,m,d,n,p,q,r,g,t,h,u,v,w,x){function k(b){const a=new l.ShaderBuilder;a.vertex.uniforms.add("proj","mat4").add("view","mat4").add("camPos","vec3").add("localOrigin","vec3");a.varyings.add("vpos","vec3");a.include(x.PathVertexPosition,b);if(0===b.output||7===b.output)a.include(e.Transform,{linearDepth:!1}),b.receiveShadows&&a.include(g.ReadShadowMap,b),a.include(t.ForwardLinearDepth,b),a.varyings.add("vnormal","vec3"),a.varyings.add("vcolor","vec4"),b.multipassTerrainEnabled&&a.varyings.add("depth",
"float"),a.vertex.code.add(c.glsl`
    ...
    `);b.multipassTerrainEnabled&&(a.fragment.include(n.ReadLinearDepth),a.include(q.multipassTerrainTest,b));7===b.output&&(a.include(d.Slice,b),a.fragment.uniforms.add("camPos","vec3"),a.fragment.uniforms.add("localOrigin","vec3"),a.fragment.uniforms.add("opacity","float"),a.fragment.code.add(c.glsl`
      void main() {
        discardBySlice(vpos);
    ...
    `),a.include(d.Slice,b),a.include(r.OutputDepth,b),a.fragment.uniforms.add("timeElapsed","float"),a.fragment.code.add(c.glsl`
        void main() {
          discardBySlice(vpos);
          outputDepth(depth);
        }
    ...
    `),a.include(d.Slice,b),a.include(p.OutputHighlight),a.fragment.code.add(c.glsl`
      void main() {
        discardBySlice(vpos);
        outputHighlight();
      }

...
"mat3"),b.multipassTerrainEnabled&&a.varyings.add("depth","float"),a.vertex.code.add(c.glsl`
    ...
    `);b.multipassTerrainEnabled&&(a.fragment.include(p.ReadLinearDepth),a.include(r.multipassTerrainTest,b));7===b.output&&(a.include(d.Slice,b),a.fragment.uniforms.add("waterColor","vec4"),a.fragment.code.add(c.glsl`
        void main() {
          discardBySlice(vpos);
    ...
    `));4===b.output&&(a.include(q.OutputHighlight),a.varyings.add("vpos","vec3"),a.vertex.code.add(c.glsl`
      void main(void) {
        if (waterColor.a < ${c.glsl.float(e.symbolAlphaCutoff)}) {
          // Discard this vertex
          gl_Position = vec4(1e38, 1e38, 1e38, 1.0);

...
function(f,b,h,k,l,m,n,p,q,r,t,u){function g(c){const a=new h.ShaderBuilder;a.extensions.add("GL_OES_standard_derivatives");a.include(u.PiUtils);a.include(q.RibbonVertexPosition,c);a.include(t.LineStipple,c);1===c.output&&a.include(r.OutputDepth,c);a.vertex.uniforms.add("proj","mat4").add("view","mat4").add("cameraNearFar","vec2").add("pixelRatio","float").add("miterLimit","float").add("screenSize","vec2");a.attributes.add("position","vec3");a.attributes.add("subdivisionFactor","float");a.attributes.add("uv0",
...
a.fragment.uniforms.add("innerWidth","float"));a.vertex.code.add(b.glsl`
...
`);a.vertex.code.add(b.glsl`
  ...
  `);c.multipassTerrainEnabled&&(a.fragment.include(m.ReadLinearDepth),a.include(p.multipassTerrainTest,c));a.include(l.Slice,c);a.fragment.uniforms.add("intrinsicColor","vec4");a.fragment.include(k.ColorConversion);a.fragment.code.add(b.glsl`
  void main() {
    discardBySlice(vpos);
  ...
  `);c.falloffEnabled&&a.fragment.code.add(b.glsl`
    ...
    `);a.fragment.code.add(b.glsl`
    if (finalColor.a < ${b.glsl.float(n.symbolAlphaCutoff)}) {
      discard;
    }

...
function(d,c,g,h,k,l,m,e,n){function f(b){const a=new h.ShaderBuilder;a.include(g.Transform,{linearDepth:!1});a.vertex.uniforms.add("proj","mat4").add("view","mat4");a.attributes.add("position","vec3");a.attributes.add("uv0","vec2");a.varyings.add("vpos","vec3");b.multipassTerrainEnabled&&a.varyings.add("depth","float");a.vertex.uniforms.add("textureCoordinateScaleFactor","vec2");a.vertex.code.add(c.glsl`
    void main(void) {
      vpos = position;
  ...
  `);a.include(l.Slice,b);b.multipassTerrainEnabled&&(a.fragment.include(m.ReadLinearDepth),a.include(n.multipassTerrainTest,b));a.fragment.uniforms.add("tex","sampler2D");a.fragment.uniforms.add("opacity","float");a.varyings.add("vTexCoord","vec2");7===b.output?a.fragment.code.add(c.glsl`
    void main() {
      discardBySlice(vpos);
    ...
    `):(a.fragment.include(k.ColorConversion),a.fragment.code.add(c.glsl`
    void main() {
      discardBySlice(vpos);
      ${b.multipassTerrainEnabled?"terrainDepthTest(gl_FragCoord, depth);":""}
